...
interface UsePetaKecamatanMapProps {
    mapboxToken: string;
    containerRef: React.RefObject<HTMLDivElement | null>;
}

export function usePetaKecamatanMap({ mapboxToken, containerRef }: UsePetaKecamatanMapProps) {
    const mapRef = useRef<mapboxgl.Map | null>(null);
    const markersRef = useRef<mapboxgl.Marker[]>([]);
    const animationFrameRef = useRef<number | null>(null);
    const onRoadClickRef = useRef<(id: string) => void>(null);
    ...
    useEffect(() => {
        ...
        map.on('load', () => {
            ...
            // Add sources
            map.addSource('kecamatan-jalan', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            map.addSource('kecamatan-desa', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            map.addSource('selected-jalan', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            ...
            map.addLayer({
                ...
                type: 'line',
                source: 'kecamatan-desa',
                paint: { 'line-color': '#10b981', 'line-width': 1, 'line-dasharray': [2, 2] }
            });
            ...
            map.addLayer({
                ...
                type: 'line',
                source: 'selected-jalan',
                paint: { 'line-color': '#ffffff', 'line-width': 2, 'line-dasharray': [0, 2, 2], 'line-opacity': 0.8 }
            });

            // Base road click listener
            map.on('click', 'jalan-line', (e) => {
                if (e.features && e.features.length > 0) {
                    const feature = e.features[0];
                    const id = feature.properties?.id || feature.id;
                    if (id && onRoadClickRef.current) {
                        onRoadClickRef.current(id.toString());
                    }
                }
            });

            const interactiveLayers = ['jalan-line', 'jalan-line-selected', 'jalan-glow', 'jalan-flow'];
            ...
            // Selected road specific click listener
            const selectedLayers = ['jalan-line-selected', 'jalan-glow', 'jalan-flow'];
    ...
    const updateKecamatanData = (geojsonJalan: GeoJSONFeatureCollection | null, geojsonDesa: any | null) => {
        ...
        if (sourceJalan && geojsonJalan) sourceJalan.setData(geojsonJalan);
        if (sourceDesa && geojsonDesa) sourceDesa.setData(geojsonDesa);
        ...
        if (geojsonDesa && geojsonDesa.features.length > 0) {
            geojsonDesa.features.forEach((feature: any) => {
                ...
                if (geometry.type === 'Polygon') {
                    geometry.coordinates[0].forEach((coord: any) => { bounds.extend(coord as [number, number]); hasBounds = true; });
                } else if (geometry.type === 'MultiPolygon') {
                    geometry.coordinates.forEach((polygon: any) => {
                        polygon[0].forEach((coord: any) => { bounds.extend(coord as [number, number]); hasBounds = true; });
                    });
                }
            });
        }
    ...
    const setOnRoadClick = (callback: (id: string) => void) => {
        onRoadClickRef.current = callback;
    };
